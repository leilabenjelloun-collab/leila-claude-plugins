...
4. **Process the remaining candidates in batches of about 5, not the whole stage in one unbroken run.** After each batch, stop and report back: candidate IDs, scores, any flags raised, any parsing concerns, and any skipped-as-already-screened candidates — then wait for the user to say "continue" before pulling the next batch. This checkpoint isn't bureaucracy for its own sake. There's no undo on a posted note, and a batch of five is small enough for a human to actually read and catch something before ten more go out the same way.
   **This default only changes on an explicit instruction** — if the user says something like "process all remaining candidates" (or clearly equivalent phrasing), drop the per-batch pause and continue through the rest of the stage without stopping, still producing a final summary at the end. Do not infer this intent from anything short of an explicit statement — an enthusiastic "looks good, keep going" after one batch means continue to the *next* batch, not abandon the checkpoint entirely. The larger the remaining candidate count, the more this override trades away the safety margin the checkpoint exists for — if the remaining count is large (dozens or more), it's reasonable to briefly confirm that's really what the user wants before proceeding uninterrupted.
